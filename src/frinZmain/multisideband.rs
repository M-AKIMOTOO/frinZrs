...
impl<W1: Write, W2: Write> Write for TeeWriter<W1, W2> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        if bytes_written1 != bytes_written2 {
            return Err(io::Error::new(ErrorKind::Other, "Partial write to one of the writers"));
        }
        Ok(bytes_written1)
    }
...
pub fn run_multisideband_analysis(args: &Args) -> Result<(), Box<dyn Error>> {
    ...
    let c_band_bp_data = if c_band_bp_path_str != "-1" {
        writeln!(tee_writer, "Reading C-band bandpass file: {}", c_band_bp_path_str)?;
        Some(read_bandpass_file(&PathBuf::from(c_band_bp_path_str))?)
    } else {
        None
    };
    let x_band_bp_data = if x_band_bp_path_str != "-1" {
        writeln!(tee_writer, "Reading X-band bandpass file: {}", x_band_bp_path_str)?;
    ...
    for rfi_pair in &args.rfi {
        ...
        if parts.len() != 2 {
            writeln!(io::stderr(), "Invalid RFI format: {}. Expected format is MIN,MAX.", rfi_pair)?;
            writeln!(tee_writer, "Invalid RFI format: {}. Expected format is MIN,MAX.", rfi_pair)?;
        ...
        if min_mhz >= max_mhz {
            writeln!(io::stderr(), "Invalid RFI range: min ({}) >= max ({}).", min_mhz, max_mhz)?;
            writeln!(tee_writer, "Invalid RFI range: min ({}) >= max ({}).", min_mhz, max_mhz)?;
        ...
        } else if min_mhz >= 1592.0 && max_mhz <= 2104.0 { // X-band range (1592-2104 MHz)
            writeln!(tee_writer, "X-band RFI range: {}-{} MHz", min_mhz, max_mhz)?;
            let converted_min = min_mhz - 1592.0;
            let converted_max = max_mhz - 1592.0;
            x_band_filtered_rfi_args_converted.push(format!("{:.0},{:.0}", converted_min, converted_max));
        } else {
    ...
    if c_band_bp_path_str != "-1" { writeln!(tee_writer, "  C-Band Bandpass: {}", c_band_bp_path_str)?; }
    writeln!(tee_writer, "  C-Band Input Delay: {} s", c_band_input_delay)?;
    writeln!(tee_writer, "  X-Band File: {:?}", x_band_path)?;
    if x_band_bp_path_str != "-1" { writeln!(tee_writer, "  X-Band Bandpass: {}", x_band_bp_path_str)?; }
    ...
    let (_c_band_complex_vec_initial, c_band_obs_time, c_band_effective_integ_time) = read_visibility_data(
        &mut c_band_cursor,
        &c_band_header,
        1,
        0,
    ...
    let (_x_band_complex_vec_initial, x_band_obs_time, x_band_effective_integ_time) = read_visibility_data(
        &mut x_band_cursor,
        &x_band_header,
        1,
        0,
    ...
    writeln!(tee_writer, "Headers match (excluding observing_frequency and station-specific fields).")?;
    ...
    writeln!(tee_writer, "C-band SNR: {:.2}", c_band_analysis_results.delay_snr)?;
    ...
    writeln!(tee_writer, "Phase difference (C - X): {:.2} deg", phase_difference_deg)?;
    ...
    let output_filename = output_dir.join(format!("{}_msbc2x{}.cor", base_filename_prefix, filename_suffix));
    ...
    for i in 0..num_sectors {
        ...
        let x_complex_vec_sector_f64: Vec<Complex<f64>> = x_complex_vec_sector.iter()
            .map(|&c| num_complex::Complex::new(c.re as f64, c.im as f64))
            .collect();
    ...
    let _c_band_freq_resolution_mhz = (c_band_header.sampling_speed as f64 / c_band_header.fft_point as f64) / 1e6;
    let _x_band_freq_resolution_mhz = (x_band_header.sampling_speed as f64 / x_band_header.fft_point as f64) / 1e6;
    ...
    let x_band_rate_profile: Vec<(f64, f64)> = x_band_analysis_results.rate_range.iter()
        .zip(x_band_analysis_results.freq_rate.iter())
        .map(|(&rate, &amp)| (rate as f64, amp as f64))
        .collect();

    let c_band_freq_resolution_mhz = (c_band_header.sampling_speed as f64 / c_band_header.fft_point as f64) / 1e6;
    ...
    let heatmap_func = move |freq_mhz: f64, rate_hz: f64| -> f64 {
        ...
        if freq_mhz >= 0.0 && freq_mhz < 512.0 + 0.01 { // C-band range
            let freq_idx = (freq_mhz / c_band_freq_resolution_mhz).round() as usize;
            if freq_idx < c_band_freq_rate_array.shape()[0] && rate_idx_c < c_band_freq_rate_array.shape()[1] {
                c_band_freq_rate_array[[freq_idx, rate_idx_c]].norm() as f64
            } else {
                0.0
            }
        } else if freq_mhz >= x_band_offset_mhz && freq_mhz < x_band_offset_mhz + 512.0 + 0.01 { // X-band range
            let freq_idx = ((freq_mhz - x_band_offset_mhz) / x_band_freq_resolution_mhz).round() as usize;
            if freq_idx < x_band_freq_rate_array.shape()[0] && rate_idx_x < x_band_freq_rate_array.shape()[1] {
    ...
    writeln!(tee_writer, "Multi-sideband .cor file saved to {:?}", output_filename)?;

    writeln!(tee_writer, "Multi-sideband frequency spectrum plot saved to {:?}", freq_plot_filename)?;
...
fn compare_headers_except_observing_frequency<W: Write>(header1: &CorHeader, header2: &CorHeader, writer: &mut W) -> io::Result<bool> {
