...
fn czt_rate_slice_direct(
    samples: &[C32],
    effective_integ_time: f32,
    rate_start_hz: f32,
    rate_step_hz: f32,
    num_rate_points: usize,
) -> Vec<C32> {
    if samples.is_empty() || num_rate_points == 0 {
        return vec![C32::new(0.0, 0.0); num_rate_points];
    }
    let dt = effective_integ_time as f64;
    let start = rate_start_hz as f64;
    let step = rate_step_hz as f64;

    let mut out = vec![C32::new(0.0, 0.0); num_rate_points];
    for k in 0..num_rate_points {
        let rate = start + step * k as f64;
        let rot = Complex::<f64>::new(0.0, -2.0 * PI * rate * dt).exp();
        let mut phase = Complex::<f64>::new(1.0, 0.0);
        let mut acc = Complex::<f64>::new(0.0, 0.0);
        for &x in samples {
            acc += Complex::<f64>::new(x.re as f64, x.im as f64) * phase;
            phase *= rot;
        }
        out[k] = C32::new(acc.re as f32, acc.im as f32);
    }
    out
}

pub fn process_fft_zoom_czt(
    complex_vec: &[C32],
    physical_length: i32,
    fft_point: i32,
    sampling_speed: i32,
    ...
    let rate_axis: Vec<f32> = (0..num_rate_points)
        .map(|k| rate_start_hz + rate_step_hz * k as f32)
        .collect();

    let complex_array = Array::from_shape_vec((rows, fft_point_half), complex_vec.to_vec())
        .unwrap_or_else(|_| Array2::<C32>::zeros((rows, fft_point_half)));
    let mut freq_rate_array = Array2::<C32>::zeros((fft_point_half, num_rate_points));
    ...
    for i in 1..fft_point_half {
        ...
        let samples: Vec<C32> = complex_array.column(i).iter().copied().collect();
        let czt_values = czt_rate_slice_direct(
            &samples,
        ...
        for (k, val) in czt_values.into_iter().enumerate() {
            freq_rate_array[[i, k]] = val * scale_factor;
        }
    }

    (freq_rate_array, rate_axis)
}
