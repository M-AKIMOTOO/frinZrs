...
use crate::utils::{safe_arg, unwrap_phase_radians};
...
fn get_phases_from_collected_data(
    ...
    for data_point in collected_data {
        ...
        let corrected_complex_vec = {
            let input_data_2d: Vec<Vec<Complex<f64>>> = data_point
                .complex_vec
                ...
                for r_idx in 0..rate_range.len() {
                    if rate_range[r_idx] >= rate_win_low && rate_range[r_idx] <= rate_win_high {
                        for d_idx in 0..delay_range.len() {
                            if delay_range[d_idx] >= delay_win_low
                                && delay_range[d_idx] <= delay_win_high
                            {
...
pub fn run_acel_search_analysis(
    ...
    for (step_idx, &degree) in acel_search_degrees.iter().enumerate() {
        ...
        println!(
            "  Updated acel: {:.9e} (Hz/s), Updated rate: {:.9e} (Hz)",
            total_acel_correct, total_rate_correct
        );
        println!(
            "  Copypaste: --acel {:.18}  --rate {:.15}",
            total_acel_correct, total_rate_correct
        );
    }
    ...
    for txt_path in generated_txt_files {
        ...
        if let Err(e) =
            plot_acel_search_result(txt_path.to_str().unwrap(), png_path.to_str().unwrap())
        {
            eprintln!(
                "Error plotting acel search result for {:?}: {}",
                txt_path, e
            );
        }
    }
