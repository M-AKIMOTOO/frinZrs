...
pub fn parse_rfi_ranges(rfi_args: &[String], rbw: f32) -> io::Result<Vec<(usize, usize)>> {
    ...
    for rfi_pair in rfi_args {
        ...
        if parts.len() != 2 {
            eprintln!("Invalid RFI format: {}. Expected format is MIN,MAX.", rfi_pair);
            process::exit(1);
        }

        let min_mhz: f32 = parts[0].parse().map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, format!("Invalid number for RFI min: {}", parts[0])))?;
        let max_mhz: f32 = parts[1].parse().map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, format!("Invalid number for RFI max: {}", parts[1])))?;
