...
fn build_b_matrix_with_map(m: usize, col_map: &HashMap<usize, usize>, rows: &[(usize, usize)]) -> DMatrix<f64> {
    let n = col_map.len();
    let mut b = DMatrix::<f64>::zeros(m, n);
    for (i, &(p, q)) in rows.iter().enumerate() {
        if let Some(&jp) = col_map.get(&p) { b[(i, jp)] += 1.0; }
        if let Some(&jq) = col_map.get(&q) { b[(i, jq)] -= 1.0; }
    }
    b
}
...
pub fn solve_antenna_tau_rate(
    ...
    if solves.is_empty() {
        ...
        return AntennaSolution { tau_s: vec![0.0; num_ant], rate_hz: vec![0.0; num_ant] };
    }
    ...
    while let Some(u) = stack.pop() {
        if visited.insert(u) {
            if let Some(nei) = adj.get(&u) {
                for &v in nei { if !visited.contains(&v) { stack.push(v); } }
            }
        }
    }
    ...
    for s in solves { all_ants.insert(s.a); all_ants.insert(s.b); }
    let _total_ants_seen = all_ants.len();
    let mut component_count = 0usize;
    let mut seen_comp: HashSet<usize> = HashSet::new();
    for &a in &all_ants {
        if seen_comp.contains(&a) { continue; }
        // BFS from a
        component_count += 1;
        let mut st = vec![a];
        while let Some(u) = st.pop() {
            if seen_comp.insert(u) {
                if let Some(nei) = adj.get(&u) {
                    for &v in nei { if !seen_comp.contains(&v) { st.push(v); } }
                }
            }
        }
    }
    ...
    if filtered.is_empty() {
        ...
        for &a in &all_ants {
            if seen_all.contains(&a) { continue; }
            let mut comp: BTreeSet<usize> = BTreeSet::new();
            let mut st = vec![a];
            while let Some(u) = st.pop() {
                if !seen_all.insert(u) { continue; }
                comp.insert(u);
                if let Some(nei) = adj.get(&u) { for &v in nei { if !seen_all.contains(&v) { st.push(v); } } }
            }
            comps.push(comp);
        }
        if let Some(best) = comps.into_iter().max_by_key(|c| c.len()) {
            let mut best_set: HashSet<usize> = HashSet::new();
            for &x in &best { best_set.insert(x); }
            filtered = solves.iter().filter(|s| best_set.contains(&s.a) && best_set.contains(&s.b)).collect();
        ...
        eprintln!("#WARN: Ignoring {} baseline(s) not connected to reference {}.", ignored, ref_ant);
    }
    ...
    if dof_tau < 0 {
        eprintln!("#WARN: Negative degrees of freedom ({}): m={} < n={}", dof_tau, m_rows, n_unknowns);
    }
    // Build sqrt(W) * B for cond estimate
    let mut bw = DMatrix::<f64>::zeros(m_rows, n_unknowns);
    for i in 0..m_rows {
        let wi = w_tau[i].sqrt();
        for j in 0..n_unknowns { bw[(i, j)] = wi * b[(i, j)]; }
    }
    {
    ...
    AntennaSolution { tau_s: tau, rate_hz: rate }
}

pub fn solve_antenna_phase(
    num_ant: usize,
    ref_ant: usize,
    solves: &[BaselineSolve],
) -> Vec<f64> {
    ...
    let mut adj: HashMap<usize, Vec<usize>> = HashMap::new();
    for s in solves { if s.w_phase > 0.0 { adj.entry(s.a).or_default().push(s.b); adj.entry(s.b).or_default().push(s.a); } }
    ...
    if visited.len() <= 1 { return vec![0.0f64; num_ant]; }
    ...
    for _ in 0..50 {
        v = &m * &v;
        // normalize
        let norm = v.iter().map(|z| z.norm_sqr()).sum::<f64>().sqrt().max(1e-12);
        v.unscale_mut(norm);
    }
    // Extract phases, anchor to reference
    let mut out = vec![0.0f64; num_ant];
    let ref_phase = if let Some(&ir) = idx_of.get(&ref_ant) { v[ir].arg() } else { 0.0 };
