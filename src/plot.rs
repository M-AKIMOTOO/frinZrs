...
pub fn cumulate_plot(
    ...
    let y_min_log = cumulate_snr.iter().cloned().filter(|&x| x > 0.0).fold(f32::INFINITY, f32::min);
    let y_axis_lower_bound = if y_min_log.is_finite() && y_min_log > 0.0 {
        y_min_log
    } else {
        0.1f32 // データに正の値がない場合や、最小値が非常に小さい場合の安全なフォールバック
    };
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .x_label_area_size(60)
        .y_label_area_size(100)
        .build_cartesian_2d((*cumulate_len.first().unwrap()..*cumulate_len.last().unwrap()).log_scale(), (y_axis_lower_bound..*cumulate_snr.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() * 1.05).log_scale())?;
...
pub fn plot_heatmap(
    ...
    for i in 0..rows {
        for j in 0..cols {
            let x = x_range.0 + (x_range.1 - x_range.0) * j as f32 / (cols - 1) as f32;
            let y = y_range.0 + (y_range.1 - y_range.0) * i as f32 / (rows - 1) as f32;
            let val = data[[i, j]];
            let normalized_val = if max_val > 0.0 { val / max_val } else { 0.0 };
            chart.draw_series(std::iter::once(Rectangle::new(
                [(x, y), (x + (x_range.1 - x_range.0) / cols as f32, y + (y_range.1 - y_range.0) / rows as f32)],
                HSLColor(((1.0 - normalized_val) * 0.7).into(), 1.0, 0.5).filled(),
            )))?;
        }
    }

    let mut colorbar = ChartBuilder::on(&root)
        .margin_top(10)
        .margin_bottom(50)
        .set_label_area_size(LabelAreaPosition::Right, 100)
        .set_label_area_size(LabelAreaPosition::Left, 0)
    ...
    let steps = 100;
    for i in 0..steps {
        let value = i as f32 / (steps - 1) as f32;
        let color = HSLColor(((1.0 - value) * 0.7).into(), 1.0, 0.5);
        colorbar.draw_series(std::iter::once(Rectangle::new(
            [(0.0, value * max_val), (1.0, (value + 1.0 / steps as f32) * max_val)],
            color.filled(),
        )))?;
    }
...
pub fn plot_dynamic_spectrum(
    complex_vec: &[Complex<f32>],
    freq_rate_array: &Array2<Complex<f32>>,
    _lag_rate_2d_data_comp: &Array2<Complex<f32>>,
    freq_range: &[f32],
    lag_range: &[i32],
    _rate_range: &[f32],
    ...
    use std::fs;
    let dynamic_spectrum_path = save_directory_path.join("dynamic_spectrum");
    fs::create_dir_all(&dynamic_spectrum_path)?;

    let time_str = obs_time.format("%Y/%j %H:%M:%S UT").to_string();

    // --- Frequency Domain Dynamic Spectrum ---
    let (rows, cols) = freq_rate_array.dim();
    let mut dynamic_spectrum_freq_time = Array2::zeros((cols, rows));
    for i in 0..rows {
        for j in 0..cols {
            dynamic_spectrum_freq_time[[j, i]] = freq_rate_array[[i, j]].norm();
        }
    }

    let freq_output_path = dynamic_spectrum_path.join(format!("{}_dynamic_spectrum_frequency.png", base_filename));
    plot_heatmap(
        &dynamic_spectrum_freq_time,
        (*freq_range.first().unwrap(), *freq_range.last().unwrap()),
        (0.0, pp as f32),
        "Frequency [MHz]",
        &format!("elapsed time (sec) since {}", time_str),
        "amplitude",
        &freq_output_path,
    )?;

    // --- Time Domain Dynamic Spectrum ---
    let mut dynamic_spectrum_lag_time = Array2::zeros((pp as usize, fft_point as usize));
    let mut planner = FftPlanner::<f32>::new();
    let fft = planner.plan_fft_inverse(fft_point as usize);

    for i in 0..pp as usize {
        let mut input_slice: Vec<RustComplex<f32>> = complex_vec[i * (fft_point / 2) as usize .. (i + 1) * (fft_point / 2) as usize]
            .iter()
            .map(|&c| RustComplex::new(c.re, c.im))
            .collect();
        input_slice.resize(fft_point as usize, RustComplex::new(0.0, 0.0));
        fft.process(&mut input_slice);

        // Shift and reverse as in Python
        let mut shifted_and_reversed: Vec<f32> = Vec::with_capacity(fft_point as usize);
        for k in 0..fft_point as usize {
            let idx = (k + fft_point as usize / 2) % fft_point as usize;
            shifted_and_reversed.push(input_slice[idx].norm());
        }
        shifted_and_reversed.reverse();

        for j in 0..fft_point as usize {
            dynamic_spectrum_lag_time[[i, j]] = shifted_and_reversed[j];
        }
    }

    let lag_output_path = dynamic_spectrum_path.join(format!("{}_dynamic_spectrum_time_lag.png", base_filename));
    plot_heatmap(
        &dynamic_spectrum_lag_time,
        (*lag_range.first().unwrap() as f32, *lag_range.last().unwrap() as f32),
        (0.0, pp as f32),
        "time lag [samples]",
        &format!("elapsed time (sec) since {}", time_str),
        "amplitude",
