...
if args.input.is_none() && args.phase_reference.is_empty() {
        ...
        println!("Running with input file: {}", input.display());
...
fn run_single_file_analysis(args: &Args) -> Result<(), Box<dyn Error>> {
    ...
    if args.add_plot {
        ...
        // Convert Vec<DateTime<Utc>> to Vec<f32> (Unix timestamps)
        let add_plot_times_f32: Vec<f32> = result.add_plot_times.iter()
            .map(|dt| dt.timestamp() as f32)
            .collect();

        add_plot(
            add_plot_filepath.to_str().unwrap(),
            &add_plot_times_f32, // Use the converted vector
            &result.add_plot_amp,
            &result.add_plot_snr,
...
fn process_cor_file(
    ...
    for l1 in 0..loop_count {
        ...
        if args.search {
            // --- SEARCH MODE ---
            let (mut coarse_freq_rate_array, padding_length) = process_fft(
                &complex_vec,
                current_length,
                header.fft_point,
                &rfi_ranges,
            ...
            let coarse_results = analyze_results(
                &coarse_freq_rate_array,
                &coarse_delay_rate_2d_data_comp,
                &header,
                current_length,
            ...
            iter_args.delay_window = vec![-16.0, 16.0];
            iter_args.rate_window = vec![-0.05, 0.05];
            ...
            for _ in 0..args.iter {
                ...
                let temp_complex_vec: Vec<C32> = temp_complex_vec_2d
                    .into_iter()
                    .flatten()
            ...
            let final_complex_vec: Vec<C32> = final_complex_vec_2d
                .into_iter()
                .flatten()
                .map(|v| Complex::new(v.re as f32, v.im as f32))
                .collect();
