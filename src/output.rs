...
pub fn write_phase_corrected_spectrum_binary(
    file_path: &Path,
    header_bytes: &[u8],
    complex_spectra: &[Vec<Complex<f32>>],
    per_sector_raw_headers: &[Vec<u8>],
) -> io::Result<()> {
    let file = File::create(file_path)?;
    let mut writer = BufWriter::new(file);
    ...
    // 2. データ部分
    for (i, spectrum) in complex_spectra.iter().enumerate() {
        // Write the raw 128-byte header for this sector
        writer.write_all(&per_sector_raw_headers[i])?;

        // 複素スペクトルの実部と虚部 (各4 byte)
        for c in spectrum {
            writer.write_f32::<LittleEndian>(c.re)?;
            writer.write_f32::<LittleEndian>(c.im)?;
        }
